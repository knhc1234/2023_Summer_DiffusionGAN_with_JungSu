...
def q_sample(x_0, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, t, noise_type='gauss', noise_std=1.0):
    ...
    #alphas_t_index = int(alphas_bar_sqrt[t])  # Convert tensor to integer index
    #alphas_t_sqrt = alphas_bar_sqrt[alphas_t_index].view(batch_size, num_channels, 1, 1)
...
class Diffusion(torch.nn.Module):
    ...
    def forward(self, x_0, noise_std=1.0):
        ...
        t = torch.from_numpy(np.random.choice(self.t_epl, size=batch_size * num_channels, replace=True)).type(torch.long).to(device) # tensor
        #print(t)
